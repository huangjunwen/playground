 ...
 *           showing: an error, or a clean check (a running command has
 *           no verdict yet, and a stale/no-verdict state renders
 *           nothing). "All Done" is not a verdict of its own: checked
 *           plus an empty goals panel composes it.
 */
...
export type VerdictView = { kind: 'error' | 'checked' };
...
export function statusCluster(state: EditorState): StatusCluster {
  ...
  if (!session.busy) {
    if (session.error !== undefined) verdict = { kind: 'error' };
    else if (session.checked) verdict = { kind: 'checked' };
  }
